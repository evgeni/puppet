  ...
  describe "when autorequiring" do
    ...
    describe "source" do
      it "should require file resource when specified with the source property" do
        source_path = '/bar'
        file = described_class.new(:path => File.expand_path("/foo"), :ensure => :file, :source => File.expand_path(source_path))
        source = described_class.new(:path => File.expand_path(source_path), :ensure => :file)
        catalog.add_resource file
